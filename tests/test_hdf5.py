...
def test_prism_response(pcic_data_portal, authorized_session_id):
    req = Request.blank('/data/bc_prism_demo/bc_tmax_review_07.nc4.html')
    req.cookies['beaker.session.id'] = authorized_session_id
...
def test_dds_response(pcic_data_portal, authorized_session_id):
    req = Request.blank('/data/canada_map/pr+tasmax+tasmin_day_CI+ANUSPLIN300+CanESM2_historical+rcp45_r1i1p1_19500101-21001231.h5.dds')
    req.cookies['beaker.session.id'] = authorized_session_id
    resp = req.get_response(pcic_data_portal)
    assert resp.status == '200 OK'
