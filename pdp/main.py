...
def initialize_backend(global_config, use_analytics=False):
    ...
    wsgi_app = PathDispatcher([
        ('^/{}/.*$'.format(bc_prism.url_base),
         data_server(global_config, bc_prism.ensemble_name)),
        ('^/{}/.*$'.format(bcsd_canada.url_base),
         data_server(global_config, bcsd_canada.ensemble_name)),
        ('^/{}/.*$'.format(vic_gen1.url_base),
         data_server(global_config, vic_gen1.ensemble_name)),
        ('^/{}/.*$'.format(gridded_observations.url_base),
         data_server(global_config, gridded_observations.ensemble_name)),
        ('^/{}/.*$'.format(bccaq_extremes.url_base),
         data_server(global_config, bccaq_extremes.ensemble_name)),
        ('^/{}/.*$'.format(hydro_stn.url_base),
