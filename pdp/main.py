...
from portals.gridded_observations import data_server as \
    gridded_observations_data_server
...
def initialize_frontend(global_config, use_auth=False, use_analytics=False):
    ...
    wsgi_app = PathDispatcher([
        ...
        ('^/{}/.*$'.format(hydro_stn.url_base),
         hydro_stn_portal(global_config)),
    ...
    if use_auth:
        ...
        atexit.register(clean_session_dir,
                        global_config['session_dir'],
                        global_config['clean_session_dir'])
...
def initialize_backend(global_config, use_auth=False, use_analytics=False):
    ...
    wsgi_app = PathDispatcher([
        ...
        ('^/{}/.*$'.format(gridded_observations.url_base),
         gridded_observations_data_server(global_config,
                                          gridded_observations.ensemble_name)),
        ('^/{}/.*$'.format(bccaq_extremes.url_base),
         bccaq_extremes_data_server(global_config,
                                    bccaq_extremes.ensemble_name)),
        ('^/{}/.*$'.format(hydro_stn.url_base),
...
def initialize_dev_server(global_config, use_auth=False, use_analytics=False):
    ...
    return DispatcherMiddleware(
        initialize_frontend(global_config, use_auth, use_analytics),
        {
