...
function update_record_length(box) {
    ...
    function success(data, textStatus, jqXHR) {
        if (req_time < last_record_length_req_time) {
            return;
        }
        box.readonly = null;
...
function netChange(map, e) {
    filterChange(net_filter, 'network', map, e);
}

function freqChange(map, e) {
    filterChange(freq_filter, 'freq', map, e);
}

function varChange(map, e) {
    filterChange(var_filter, 'stn_var', map, e);
}
