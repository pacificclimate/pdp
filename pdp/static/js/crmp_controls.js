function createVariableOptions(map) {
    var variableData = { '': pdp.mkOpt('All'), 
			 ...
			 Wind: pdp.mkOptGroup({ 'wind_speed_mean': pdp.mkOpt('Wind Speed (Mean)', 'Scalar speed of wind averaged over all measurements taken over the observational period as defined by the observational frequency.'), 
					    ...
					    'wind_from_direction_standard_deviation': pdp.mkOpt('Wind Direction (Std Dev)' /* NO HELP */), }),
    ...
    var varSel = pdp.getSelectorWithHelp('Climate Variables', 'variables', 'input-var', 'input-var', '', variableData, 'View variable definitions', 450, 450);
...
function createNetworkOptions(map) {
    ...
    var netSel = pdp.getSelectorWithHelp('Network Name', 'network-name', 'network-name', 'input-network', '', networkOptionData, 'View table of network names', 800, 450, 'table', getNetworkHelp);
...
function createFrequencyOptions(map) {
    ...
    var freqSel = pdp.getSelectorWithHelp('Observation Frequency', 'frequency', 'input-freq', 'input-freq', '', frequencyOptionData, 'View frequency explanations', 450, 450);
...
var downloadMetadata = function get_wms_wfs(e, map) {
    var select_elem = e.target.parentElement.parentElement.childNodes[1].childNodes[1];
